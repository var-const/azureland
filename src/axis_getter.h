...
template <typename T> T& AxisGetter::ThisAxis(ci::Vec2<T>& p) const
{
    return m_OwnAxis == Axes::X ? p.x : p.y;
}

template <typename T> const T& AxisGetter::ThisAxis(const ci::Vec2<T>& p) const
{
    return m_OwnAxis == Axes::X ? p.x : p.y;
}

template <typename T> T& AxisGetter::OtherAxis(ci::Vec2<T>& p) const
{
    return m_OwnAxis == Axes::X ? p.y : p.x;
}

template <typename T> const T& AxisGetter::OtherAxis(const ci::Vec2<T>& p) const
{
    return m_OwnAxis == Axes::X ? p.y : p.x;
}
